...
즉, $x_{t}=\sqrt{1-\beta _{t}} * x_{t-1} + \sqrt{\beta _{t}}*\varepsilon$ 으로 variance를 1로 맞춰주면서 딥러닝 학습이 가능하도록 작은 노이즈를 더해주는, reparameterization trick을 이용한다. input도 normalizaion을 통해 variance가 1이 되도록 맞춰줘야 된다. (안 맞추고 하면 어떻게 될지 궁금하긴 하다) 이 때, $\beta _{t}$는 학습을 할 수도 있겠지만 미리 정해진 상수로 정의해서 forward process에는 학습이 전혀 필요하지 않게 된다. (논문에서는 학습을 해봤는데 상수로 둔 거랑 별 차이가 없었다고 말했다) 또한 $\beta _{t}$의 scheduling을 1e-4~0.02로 점차 증가시키는데 (linear, sigmoid, cosine 등 여러가지 scheduling이 또 가능하겠다. linear는 너무 빠르게 noise로 변하는 문제가 있어서 cosine 등의 scheduling을 도입했다고 한다.), 처음에는 noise를 조금씩 더하다가 점차 많이 더해가는 느낌이다.

한 방에 적으면, $$q(X_{t}\mid X_{0})=N(X_{t}:\sqrt{\overline{\alpha _{t}}}X_{0}, (1-\overline{\alpha _{t}})*I) \; where \; \alpha _{t}=1-\beta_{t}, \overline{\alpha _{t}}=\prod_{i=1}^{t}\alpha _{i}$$
