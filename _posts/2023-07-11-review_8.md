...
$$\textup{Self-Attention}(Q,K,V)=\textup{Attention}(QW_{q},KW_{k},VW_{v})W_{o} \\ where\;\;\textup{Attention}(Q,K,V)=\textup{softmax}(\frac{QK^{T}}{\sqrt{d_{model}}})V$$
...
## Ablation Studies
1. Pre-trained Transformer에 어떤 weight matrice를 adapt해야 되는가?
...
3. Adaptation Matrix $\Delta W$는 $W$와 어떤 관계인가?

![Ablation3]({{site.url}}/images/review/LoRA/7.png)
