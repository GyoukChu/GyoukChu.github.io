...
N(\mathbf{x}(t);\mathbf{x}(0),[\sigma_{max}^{2}-\sigma_{min}^{2}]\mathbf{I})\;\;(\textup{VE SDE})\\ 
N(\mathbf{x}(t);\mathbf{x}(0)e^{-\frac{1}{2}\int_{0}^{t}\beta(s)ds},\mathbf{I}-\mathbf{I}e^{-\int_{0}^{t}\beta(s)ds})\;\;(\textup{VP SDE})\\ 
N(\mathbf{x}(t);\mathbf{x}(0)e^{-\frac{1}{2}\int_{0}^{t}\beta(s)ds},\mathbf{I}[1-e^{-\int_{0}^{t}\beta(s)ds}]^{2})\;\;(\textup{sub-VP SDE})
\end{matrix}\right.$$
...
이후에는 더 좋은 reverse SDE/ODE integrator는 무엇인가에 대한 연구들이 이어지게 된다. [Zhang & Chen, 2022](https://arxiv.org/abs/2204.13902) [Lu et al., 2022](https://arxiv.org/abs/2206.00927)
